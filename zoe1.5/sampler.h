...
const int bufSize = 3*1024;          // buffer size in bytes. You can change this data.
   ...
   boolean buffill()
   {
     ...
     if(playSample)
     {
       ...
       else if(bufRead==lastBuf)
       {
         ...
         if(me<=0) 
  	     {
  	       Serial.println("error reading file during buffer fill \r\n");
  	       playSample = false;
  	       myFile.close();
  	     }
      	}
...
// Compute the position of the next sample. Must be called in the sound generating loop, before the method "output()"   
   void next()
   {
     if(playSample)  
     {
       // If the wave file is stereo, we take only one side (left). So increase buffer index by 1 or 2. 
...
// Read the wave file at a position. Returns the volume (12 bits)
  int16_t output()
  {
    ...
    if(playSample)
    {
      ret = (buf[bufRead][int(indBuf)]>>4)*volume;
    }
